...
/*
void memset(char *p, int len, int v) {
    int i = 0;
    for (i = 0; i < len; i++) {
        p[i] = v;
    }
...
//static  int fifobuf[128];
//static  char cmdline[30];
...
void console_task(struct SHEET *sheet, int memtotal) {
    ...
    for(;;) { 
        ...
        if (fifo8_status(&task->fifo) == 0) {
            ...
            if (cursor_c >= 0) {
                  if (hlt == 1) {
              //char *p = intToHexStr(g_Console.cur_x);
             //showString(shtctl, sheet, 8, 48, COL8_FFFFFF, p);
              }

             if (hlt == 2) {
//             char *p1 = intToHexStr(g_Console.cur_y);
  //           showString(shtctl, sheet, 8, 64, COL8_FFFFFF, p1);
              }
           ...
           //  showString(shtctl, sheet, 8, 64, COL8_FFFFFF, p1);
                ...
                //boxfill8(sheet->buf, sheet->bxsize, COL8_FFFFFF, 16,
               // 48, 16 + 7, 48 + 15);
               // sheet_refresh(shtctl, sheet, 16, 48, 16+8, 48 + 16);
              }
